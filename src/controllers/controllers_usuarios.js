...
// Modificação no endpoint do servidor
controllerUsuarios.post("/usuarios/login", async function (request, response) {
  try {
    ...
    db.query(sqlSelect, [request.body.email_user, md5Hash], function (err, result) {
      if (err) {
        return response.status(500).send(err);
      } else {
        if (result.length > 0) {
          ...
          db.query(sqlUpdate, [currentDate, user_id], function (updateErr, updateResult) {
            if (updateErr) {
          ...
          response.status(401).json({ error: 'Nenhum resultado encontrado.' });
        }
      }
    });
  } catch (err) {
...
controllerUsuarios.post(
  ...
  async function (request, response) {
    try {
      ...
      const sql =
        "INSERT INTO usuarios (nome_user, email_user, senha_user, token_user, data_cadastro_user,quantidade_acesso) VALUES (?, ?, ?, ?, ?, ?)";
      db.query(
        sql,
        [
